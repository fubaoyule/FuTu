...
class InfoModifyView: UIView,UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    func initNavigation() -> Void {
        ...
        let titleLabel = baseVC.labelCreat(frame: titleFrame, text: "资料更改", aligment: .center, textColor: .white, backgroundcolor: .clear, fonsize: fontAdapt(font: isPhone ? 17 : 12))
...
//        let backBtnImg = UIImageView(frame: CGRect(x: 10, y: 12, width: 12, height: 20))
        ...
        //save button
        let saveFrame = CGRect(x: width - adapt_W(width: 80), y: adapt_H(height: 10) + 20, width: 80, height: adapt_H(height: 40))
    ...
    func initInputView() -> Void {
        ...
        codeBtn.setTitleColor(UIColor.colorWithCustom(r: 27, g: 123, b: 233), for: .normal)
...
//            alertLable.tag = personBtnTag.AlertLableTag.rawValue
            ...
            let confirmBtn = baseVC.buttonCreat(frame: confirmBtnFrame, title: "确  认", alignment: .center, target: self, myaction: #selector(self.btnAct(sender:)), normalImage: nil, hightImage: nil, backgroundColor: .colorWithCustom(r: 27, g: 123, b: 233), fonsize: fontAdapt(font: isPhone ? 17 : 12), events: .touchUpInside)
    ...
    func tapGuestureAct(tap:UITapGestureRecognizer) -> Void {
