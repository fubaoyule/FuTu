...
class PersonView: UIView {
    ...
    func initNavigationBar() -> Void {
        ...
        if let infoHasBind = userDefaults.value(forKey: userDefaultsKeys.userInfoIsBasicBound.rawValue) {
            if !(infoHasBind as! Bool) {
                // edit button
                let editFrame = CGRect(x: width - adapt_W(width: isPhone ? 80 : 40), y: adapt_H(height: 10) + 20, width: adapt_W(width: isPhone ? 80 : 60), height: adapt_H(height: isPhone ? 40 : 30))
    ...
    func initScrollView() -> Void {
        ...
        for i in 0 ..< model.infoLabel.count - 2 {
            ...
            let balanceTextWordSize = textWord.size(attributes: [NSFontAttributeName :  UIFont.systemFont(ofSize: fontAdapt(font: isPhone ? 14 : 10))])
