...
class TransferView: UIView {
    ...
    func initScrollView() -> Void {
        ...
        let scrollContentHeight:CGFloat = adapt_H(height:(isPhone ? (self.model.gameViewTop + tabBarHeight + 100) : (120 + tabBarHeight + 50))) + 7 * gameViewHeight * 0.6
...
//            let game = (i == 0 ? "platformGame_SB" : globleGameUserDefaults[i - 1])
            ...
            let balanceValueSize = balanceValue.size(attributes: [NSFontAttributeName :  UIFont.systemFont(ofSize: fontAdapt(font: isPhone ? 20 : 16))])
            ...
            let balanceTextWordSize = balanceTextWord.size(attributes: [NSFontAttributeName :  UIFont.systemFont(ofSize: fontAdapt(font: isPhone ? 13 : 10))])
...
//            gameView.insertSubview(balanceText, aboveSubview: imgView)
    ...
    func initChangedScrollView() -> Void {
        ...
        isChanged = true
        let gameViewHeight:CGFloat = adapt_H(height: (isPhone ? 112 : 80))
        let scrollContentHeight:CGFloat = adapt_H(height:(isPhone ? (self.model.gameViewTop + tabBarHeight) : (120 + tabBarHeight)) + 30) + 8 * gameViewHeight
        scroll.contentSize = CGSize(width: frame.width, height: scrollContentHeight)
    ...
    //层叠状态下游戏条点击事件
    func gameTouchAct(sender:UITapGestureRecognizer) -> Void {
