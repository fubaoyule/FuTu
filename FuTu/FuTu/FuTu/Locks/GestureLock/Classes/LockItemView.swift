...
class LockItemView: UIView {
    var direct: LockItemViewDirect? {
        willSet {
            if let newValue = newValue {
                angle = CGFloat(Double.pi / 4) * CGFloat(newValue.rawValue - 1)
                setNeedsDisplay()
            }
        }
    }
    ...
    fileprivate var calRect: CGRect {
        ...
        get {
            if storeCalRect.equalTo(CGRect.zero) {
                ...
                //定义圆环的位置和大小
                //self.storeCalRect = CGRect(x: originXY + (sizeWH * 0.1), y: originXY + (sizeWH * 0.1), width: sizeWH * 0.8, height: sizeWH * 0.8)
            }
            return storeCalRect
        }
    }
    ...
    override func draw(_ rect: CGRect) {
        if let context = UIGraphicsGetCurrentContext() {
            ...
            //外环填充
            circleInfill(context, rect: rect)
    ...
    //外圆环填充
    func circleInfill(_ context: CGContext, rect: CGRect) {
        //新建路径：外环
        let loopPath = CGMutablePath()
