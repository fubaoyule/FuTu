...
open class LockController: UIViewController {
    ...
    fileprivate lazy var imageView: LockImageView = {
        return LockImageView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height), options: self.options)
    }()
    ...
    fileprivate func onPrepare() {
        ...
        view.insertSubview(imageView, at: 0)//将imageView放到底层
