...
class WalletHubViewController: UIViewController,UIScrollViewDelegate,BtnActDelegate,UIAlertViewDelegate {
    ...
    func btnAct(btnTag: Int) {
        ...
        case walletHubTag.BailoutsBtnTag.rawValue:
            tlPrint(message: "昨日救援")
            bailoutBtnAct(isYestoday: true)
            let bailoutBtn = self.walletHubView.viewWithTag(btnTag) as! UIButton
            bailoutBtn.isEnabled = false
        case walletHubTag.BailoutsBtnTag.rawValue + 1:
            tlPrint(message: "今日救援")
            //禁用今日救援按钮
            let bailoutBtn = self.walletHubView.viewWithTag(btnTag) as! UIButton
            bailoutBtn.isEnabled = false
            bailoutBtnAct(isYestoday: false)
        default:
            tlPrint(message: "no such case")
        }
    }
    
    func bailoutBtnAct(isYestoday:Bool) -> Void {
        //救援金入口
        self.checkLoginStatus {
            self.model.getBailoutsInfo(yestoday: isYestoday, success: { (response) in
                tlPrint(message: "response : \(response)")
                var message = ""
                ...
                let alert = UIAlertView(title: isYestoday ? "昨日救援" : "今日救援", message: message, delegate: nil, cancelButtonTitle: "确 定")
                alert.show()
                //启用按钮
                let bailoutBtn = self.walletHubView.viewWithTag(walletHubTag.BailoutsBtnTag.rawValue + (isYestoday ? 0 : 1)) as! UIButton
                bailoutBtn.isEnabled = true
                if amount > 0 {
                    self.refreshAct()
                }
