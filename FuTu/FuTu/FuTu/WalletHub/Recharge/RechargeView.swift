...
class RechargeView: UIView,UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func initNavigationBar() -> Void {
        ...
        setLabelProperty(label: titleLabel, text: "存款", aligenment: .center, textColor: .white, backColor: .clear, font: 17)
    ...
    let payTypeBtnText = ["微信转账", "QQ扫码", "微信扫码", "支付宝扫码", "在线充值", "支付宝转账", "网银", "点卡支付"]
    ...
    func initInputTable() -> Void {
        ...
        self.tableView = baseVC.viewCreat(frame: tableFrame, backgroundColor: .colorWithCustom(r: 244, g: 244, b: 244))
        ...
        let alertWords = "     注：单笔充值限额（20元－5000元）"
        //alert label
        let alertFrame = CGRect(x: 0, y: 0, width: tableFrame.width, height: adapt_H(height: isPhone ? 45 : 25))
        let alertLabel = baseVC.labelCreat(frame: alertFrame, text: "", aligment: .left, textColor: .colorWithCustom(r: 245, g: 63, b: 0), backgroundcolor: .colorWithCustom(r: 244, g: 244, b: 244), fonsize: fontAdapt(font: isPhone ? 14 : 9))
        ...
        hintString.addAttribute(NSForegroundColorAttributeName, value: UIColor.colorWithCustom(r: 245, g: 63, b: 0), range: range1)
        let range2 = NSRange(location: alertWords.characters.count - 6, length: 4)
        hintString.addAttribute(NSForegroundColorAttributeName, value: UIColor.colorWithCustom(r: 245, g: 63, b: 0), range: range2)
        ...
        inputTextField = baseVC.textFieldCreat(frame: textFrame, placeholderText: "请输入金额", aligment: .left, fonsize: fontAdapt(font: isPhone ? 20 : 11), borderWidth: adapt_H(height: 0.5), borderColor: .colorWithCustom(r: 169, g: 169, b: 169), tag: rechargeTag.InputTextFieldTag.rawValue)
...
//    }
    ...
    //充值金额上面的提示信息
    func initAlertLabel(lowLimit:Int,hightLimit:Int) -> Void {
        ...
        //label 变颜色
//        let hintString = NSMutableAttributedString(string: alertWords)
//        let range1 = NSRange(location: alertWords.characters.count - (hightLimit + lowLimit + 4), length: lowLimit)
//        hintString.addAttribute(NSForegroundColorAttributeName, value: UIColor.colorWithCustom(r: 245, g: 63, b: 0), range: range1)
//        let range2 = NSRange(location: alertWords.characters.count - (hightLimit + 2), length: hightLimit)
//        hintString.addAttribute(NSForegroundColorAttributeName, value: UIColor.colorWithCustom(r: 245, g: 63, b: 0), range: range2)
    ...
    func initUIPickerView() -> UIPickerView {
        ...
        pickerConfirmBtn = baseVC.buttonCreat(frame: confirmFrame, title: "选 择", alignment: .center, target: self, myaction: #selector(self.btnAct(sender:)), normalImage: nil, hightImage: nil, backgroundColor: .colorWithCustom(r: 27, g: 123, b: 233), fonsize: fontAdapt(font: isPhone ? 20 : 13), events: .touchUpInside)
...
//            return 0
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        hintString.addAttribute(NSForegroundColorAttributeName, value: UIColor.colorWithCustom(r: 10, g: 114, b: 232), range: range)
        hintString.addAttribute(NSFontAttributeName, value: UIFont.systemFont(ofSize: fontAdapt(font: isPhone ? 12 : 8)) , range: range)
    ...
    func initNextBtn() -> Void {
        ...
        let nextFrame = CGRect(x: adapt_W(width: isPhone ? 42 : 50), y: adapt_H(height: isPhone ? 530 : 360), width: width - adapt_H(height: isPhone ? 84 : 100), height: adapt_H(height: isPhone ? 50 : 30))
        nextBtn = baseVC.buttonCreat(frame: nextFrame, title: "下一步", alignment: .center, target: self, myaction: #selector(self.btnAct(sender:)), normalImage: nil, hightImage: nil, backgroundColor: .colorWithCustom(r: 10, g: 114, b: 232), fonsize:fontAdapt(font: isPhone ? 17 : 11), events: .touchUpInside)
    ...
    func initWarmingLabel() {
        ...
        //alert label
        let warmingFrame = CGRect(x: adapt_W(width: isPhone ? 42 : 50), y: adapt_H(height: isPhone ? 565 : 400), width: width - adapt_H(height: isPhone ? 84 : 100), height: adapt_H(height: isPhone ? 80 : 50))
