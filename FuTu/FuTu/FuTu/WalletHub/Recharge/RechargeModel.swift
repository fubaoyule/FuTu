...
//    case OnlinePayBtnTag = 10, WechatScanPayBtnTag, AliScanPayBtnTag, QQPayBtnTag, WechatPayBtnTag, AliPayBtnTag, UnionPayBtnTag, TimeCardPayBtnTag
    case WechatPayBtnTag = 10, QQPayBtnTag, WechatScanPayBtnTag, AliScanPayBtnTag, OnlinePayBtnTag, AliPayBtnTag, UnionPayBtnTag, TimeCardPayBtnTag
...
//    let bankCode = ["online","weixin","alipay"]
    ...
    //支付类型数组
    let payTypeArray = [PayType.WeChatPay,
                        PayType.QQPay,
    ...
    let limitAmount = [PayType.WeChatPay:isRelease ? [20,50000] : [10,10000],
                       PayType.QQPay:isRelease ? [31,5000] : [10,10000],
                       PayType.WechatScanPay:isRelease ? [31,1000] : [10,10000],
                       PayType.AliScanPay:isRelease ? [51,5000] : [10,10000],
                       ...
                       PayType.TimePay:isRelease ? [50,1000] : [10,10000]]
    ...
    func nextBtnAct(rechargeView:UIView, payType:PayType) -> Bool {
        ...
        let lowLimit:Int = (limitAmount[payType]!)[0]
        let hightLimit:Int = (limitAmount[payType]!)[1]
        ...
        if payType == PayType.WechatScanPay || payType == PayType.QQPay || payType == PayType.AliScanPay {
            if (Int(value) % 10) == 0 {
...
//    }
    ...
    func getDepositActivesList(success:@escaping([Dictionary<String,Any>])->(),failure:@escaping(()->())) -> (Void){
        ...
        let url = "Active/GetDepositActivesList"
...
//        let param = ["Amount":amount,"ActCode":activityCode]
    ...
    func unionPayRequest(amount:String,activityCode:String,success:@escaping((Dictionary<String,Any>))->(),failure:@escaping(()->())) -> Void {
        ...
        futuNetworkRequest(type: .post, serializer: .http, url: "Transact/FastDeposite", params: param ?? ["":""], success: { (response) in
            ...
            let unionPayDic = (string!).objectFromJSONString() as! Dictionary<String, Any>
