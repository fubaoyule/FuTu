...
class RechargeViewController: UIViewController,UIScrollViewDelegate, UITextFieldDelegate,BtnActDelegate,UIAlertViewDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if self.payType ==  PayType.WeChatPay {
            self.model.getWechatAccount(success: { (wechatNum) in
                ...
                self.rechargeView.initWechatAlertLabel(textString: "     添加微信：\(wechatNum)，转账并备注富宝账号！")
            }, failure: {
    ...
    func choosePayTypeAct(btnTag:Int) -> Void {
        ...
        if btnTag ==  rechargeTag.WechatPayBtnTag.rawValue {
            ...
            self.rechargeView.initAlertLabel(lowLimit: self.model.limitAmount[self.payType]![0], hightLimit: self.model.limitAmount[self.payType]![1])
        }
        
        
        //修改温馨提示内容
        let warmingWords = [PayType.WeChatPay:"",
                            PayType.QQPay:"*温馨提示：如果无法完成充值，建议使用“微信转账”",
                            PayType.WechatScanPay:"*温馨提示：若提交订单失败请等待90秒后重试！建议使用“微信转账”、“QQ扫码”",
                            PayType.AliScanPay:"*温馨提示：若提交订单失败请等待90秒后重试！建议使用“微信转账”、“QQ扫码”",
                            PayType.OnlinePay:"",
                            PayType.AliPay:"",
                            PayType.UnionPay:"",
                            PayType.TimePay:""]
    ...
    func nextBtnAct() -> Void {
        ...
        if self.payType == PayType.WeChatPay {
            ...
            return
        } else if self.payType == PayType.UnionPay || self.payType == PayType.AliPay || self.payType == PayType.TimePay {
