...
class WithdrawView: UIView {
    ...
    func initNavigationBar() -> Void {
        ...
        setLabelProperty(label: titleLabel, text: "取款", aligenment: .center, textColor: .white, backColor: .clear, font: 17)
...
//        hintString.addAttribute(NSForegroundColorAttributeName, value: UIColor.colorWithCustom(r: 245, g: 63, b: 0), range: range3)
        ...
        amountString.addAttribute(NSForegroundColorAttributeName, value: UIColor.colorWithCustom(r: 245, g: 63, b: 0), range: range)
        amountString.addAttribute(NSFontAttributeName, value: UIFont.systemFont(ofSize: fontAdapt(font: 17)), range: range)
    ...
    func initNextBtn() -> Void {
        ...
        self.nextBtn = baseVC.buttonCreat(frame: nextFrame, title: "下一步", alignment: .center, target: self, myaction: #selector(self.btnAct(sender:)), normalImage: nil, hightImage: nil, backgroundColor: .colorWithCustom(r: 10, g: 114, b: 232), fonsize:fontAdapt(font: 17), events: .touchUpInside)
    ...
    func btnAct(sender:UIButton) -> Void {
        ...
        if sender.tag == withdrawTag.WithdrawNextBtnTag.rawValue {
            if inputTextField.text == nil || inputTextField.text == "" {
                ...
                let alert = UIAlertView(title: "输入有误", message: "单笔可提现金额为100至20000元，请核对后再试", delegate: nil, cancelButtonTitle: "确 定")
