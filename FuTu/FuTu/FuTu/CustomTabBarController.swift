...
class CustomTabBarController: UITabBarController,UIAlertViewDelegate {
    ...
    let itemImages = ["home_toolbar_home1","home_toolbar_preferent1","home_toolbar_transfer1","home_toolbar_service1","home_toolbar_my1"]
    let itemImagesSelect = ["home_toolbar_home2","home_toolbar_preferent2","home_toolbar_transfer2","home_toolbar_service2","home_toolbar_my2"]
...
//    }
    func initTabSubViewController(index:Int?) -> Void {
        ...
        if index == nil {
            self.homeVC = HomeViewController()
            self.preferentVC = PreferentViewController()
            self.transferVC = TransferViewController(isFromTab: true)
            self.serviceVC = ServiceViewController()
            self.selfVC = SelfViewController()
            
            
            self.tabBarVC = [homeVC,preferentVC,transferVC,serviceVC,selfVC]
            self.setViewControllers(tabBarVC, animated: true)
...
//        initCustomTabbar()
    ...
    func initCustomTabbar() -> Void {
        ...
        let itmeNames = ["富宝首页","优惠","转账","客服","个人"]
        ...
        label1.textColor = UIColor.colorWithCustom(r: 211, g: 18, b: 0)
        
    }
    
    
    func tabBtnAct(sender:UIButton) -> Void {
        ...
        if sender.tag == 2 || sender.tag == 4 {
            self.checkLoginStatus(hasLogin: { 
                ...
                for i in 0 ... 4 {
                    if i == sender.tag {
                        ...
                        self.labels[i].textColor = UIColor.colorWithCustom(r: 211, g: 18, b: 0)
            ...
            for i in 0 ... 4 {
                if i == sender.tag {
                    ...
                    labels[i].textColor = UIColor.colorWithCustom(r: 211, g: 18, b: 0)
