...
enum ServiceSearchTag:Int {
    case backBtnTag = 10,allQuestionsBtnTag
}

class ServiceSearchModel: NSObject {
...
//         ["允许我在富途娱乐(TOOBET)进行游戏吗？","在您注册和投注前，请务必确保您已年满18周岁并同意我们的服务条款和隐私政策。不同的国家或地区有不同的监管条例，请在投注前遵循当地法规与法律。"]]
    ...
    func searchAnswerByKeywords(key:String) -> Array<Int>? {
        ...
        for i in 0 ... answerInfo.count - 1 {
            tlPrint(message: "i: \(i)")
            if (answerInfo[i][0] ).components(separatedBy: key).count > 1 {
                result.append(i)
